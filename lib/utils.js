...
function mergeAllXPData(projectData, indieData, cabsData, bountyData, btData, stackXpData) {
  ...
  for (var key in stackXpData) {
    const stackXp = stackXpData[key];
    if (stackXp && stackXp.name) {
      mergedData.push({
        name: stackXp.name,
        skill: stackXp.skill,
        xp: stackXp.xp || 0,
...
function getLeaderboardData(projectData, indieData, cabsData, bountyData, btData, stackXpData) {

  const mergedData = mergeAllXPData(projectData, indieData, cabsData, bountyData, btData, stackXpData);
